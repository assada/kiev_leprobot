    ...
    topByDays(db, chat) {
        ...
        return new Promise(function (fulfill) {
            if (t.cache.get('topByDays') === null) {
                const sql = 'SELECT count(id) as count, DAY(createdAt) as day, MONTH(createdAt) as month, YEAR(createdAt) as year FROM messages WHERE chat = ' + chat + ' GROUP BY day, month, year ORDER BY day ASC, month ASC, year ASC LIMIT 10';
